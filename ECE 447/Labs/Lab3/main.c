...
void main(void)
{
  ...
  //-------------------------------------------------
  // set interrupt on btn SW1 and SW2 and define interrupt routine
  P1DIR = 0; // set all pins to input
  P1IE |= BIT0|BIT1; // interrupt on bit 0 and 1 (btn SW1 and SW2)
  ...
  TACCTL0 = CCIE;
  P2DIR = 0xFF; // all output, data to 3 7-segment displays
  P6DIR = 0xFF; // all output, use to switch data to each 7-segment displays
